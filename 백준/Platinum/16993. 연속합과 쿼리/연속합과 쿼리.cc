...
private:
	...
	Data get_max_sum(const int target_begin, const int target_end, const int begin, const int end, const int idx) const {
		...
		const int mid = (begin + end) / 2;
		const Data left_data = get_max_sum(target_begin, target_end, begin, mid, idx * 2);
		const Data right_data = get_max_sum(target_begin, target_end, mid, end, idx * 2 + 1);
...
public:
	...
	int get_max_sum(const int target_begin, const int target_end) const {
		return this->get_max_sum(target_begin, target_end, 0, this->N, 1).total_sum;
	}
};
...
int main(void) {
	...
	for (int a = 0; a < M; ++a) {
		...
		const int result = segtree.get_max_sum(i - 1, j);
		printf("%d\n", result);
	}
